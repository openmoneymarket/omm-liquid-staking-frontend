...
export const LEDGER_WAIT_ADDRESS =
  "Waiting for you to confirm the address on your Ledger device... (60 seconds timeout)";
...
export const LEDGER_UNABLE_TO_SIGN_TX =
  "Couldn't sign the transaction with your Ledger device. Make sure it's connected and try again.";
...
export const SUCCESS_UNSTAKE_SICX = (stakeIcxAmount: BigNumber) =>
  `Unstaked ${numToUsLocaleString(stakeIcxAmount.dp(2))} sICX.`;
...
export const SUCCESS_LOCK_OMM = (lockingAction: OmmLockingPayload) =>
  `${lockingAction.amount} OMM locked until ${timestampInMillisecondsToPrettyDate(lockingAction.lockingTime)}.`;
export const SUCCESS_INCREASE_LOCK_TIME = (lockingAction: OmmLockingPayload) =>
  `OMM locked until ${timestampInMillisecondsToPrettyDate(lockingAction.lockingTime)}.`;
export const SUCCESS_INCREASE_LOCKED_OMM = (lockingAction: OmmLockingPayload) =>
  `${lockingAction.amount} OMM locked until ${timestampInMillisecondsToPrettyDate(lockingAction.lockingTime)}.`;
export const SUCCESS_INCREASE_LOCK_TIME_AND_AMOUNT = (lockingAction: OmmLockingPayload) =>
  `${lockingAction.amount} OMM locked until ${timestampInMillisecondsToPrettyDate(lockingAction.lockingTime)}.`;
export const SUCCESS_CLAIM_AND_APPLY_BOMM_BOOST = (ommClaimed: BigNumber | number) =>
  `Claimed ${numToUsLocaleString(roundDownTo2Decimals(ommClaimed))} OMM. \nbOMM boost applied.`;
export const SUCCESS_APPLY_BOMM_BOOST = "bOMM boost applied.";
export const SUCCESS_UNSTAKE_OMM = (stakingAction: StakingAction | ManageStakedIcxAction) =>
  `${stakingAction.amount} OMM unstaking.`;
...
export const NEW_PROPOSAL_MIN_BOMM_REQUIRED = (minBommRequired: BigNumber) =>
  `You need at least ${numToUsLocaleString(roundDownTo2Decimals(minBommRequired))} bOMM to propose a change.`;
...
export const NEW_PROPOSAL_INVALID_PARAMETER = (requiredType: ScoreParamType) =>
  `Invalid parameter type. Should be of type: ${requiredType}!`;
