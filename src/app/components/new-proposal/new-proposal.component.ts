...
export class NewProposalComponent extends BaseClass implements OnInit, OnDestroy {
  ...
  constructor(public persistenceService: StoreService,
  ...
  subscribeToDelegationbOmmWorkingTotalSupplyChange(): void {
    this.delegationbOmmWorkingTotalSupplySub = this.stateChangeService.delegationbOmmTotalWorkingSupplyChange$.subscribe(value => {
  ...
  private refreshMinBommRequiredForProposal(): void {
    if (this.voteDefinitionCriterion.gt(0) && this.delegationbOmmWorkingTotalSupply.gt(0)) {
      this.minBOmmRequiredForProposal = this.delegationbOmmWorkingTotalSupply.multipliedBy(this.voteDefinitionCriterion);
    }
  }
