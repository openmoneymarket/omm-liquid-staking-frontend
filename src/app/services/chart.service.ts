...
export class ChartService {
  ...
  // returns last value
  initStakingAprChart(stakingAprChartHtmlElement: any, stakingAprChart: any, liquidStakingStats: LiquidStakingStats[]): { chart: any, lastValue: number } | undefined {
    ...
    return stakingAprData.length > 0 ? { chart: stakingAprChart, lastValue: stakingAprData[stakingAprData.length - 1].value } : undefined;
  }

  initUnstakingApyChart(unstakingChartHtmlElement: any, unstakingChart: any, liquidStakingStats: LiquidStakingStats[]): { chart: any, lastValue: number } | undefined {
    // clear charts if they are already established
    ...
    return unstakingData.length > 0 ? { chart: unstakingChart, lastValue: unstakingData[unstakingData.length - 1].value } : undefined;
  }
