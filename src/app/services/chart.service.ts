...
export class ChartService {
  ...
  initStakingApyChart(stakingApyChartHtmlElement: any, stakingApyChart: any): void{
    // clear charts if they are already established
    stakingApyChart?.remove();

    // TODO use real data
    const stakingApyData: { time: string, value: number }[] = this.mockStakingApyData();

    if (stakingApyData.length > 0) {
      // create chart in html element
      stakingApyChart = LightweightCharts.createChart(stakingApyChartHtmlElement, this.stakingApyChartOptions());

      // add area series
      const supplyChartAreaSeries = stakingApyChart.addAreaSeries(this.stakingApyChartAreaSeries());

      // set data to series
      supplyChartAreaSeries.setData(stakingApyData);
    }
  }

  initUnstakingApyChart(unstakingChartHtmlElement: any, unstakingChart: any): void{
    // clear charts if they are already established
    unstakingChart?.remove();

    // TODO use real data
    const unstakingData: { time: string, value: number }[] = this.mockUnstakingData();
    ...
    if (unstakingData.length > 0) {
      ...
      // add area series
      const supplyChartAreaSeries = unstakingChart.addAreaSeries(this.unstakingChartAreaSeries());

      // set data to series
      supplyChartAreaSeries.setData(unstakingData);
    }
  }
  ...
  mockStakingApyData(): { time: string, value: number }[] {
    return [
